...
export default function ForgotPassword() {
  ...
  const validateEmail = (emailVal) => {
    if (!emailVal || emailVal.trim() === "") return "Email is required";
    return "";
  };

  const handleBlur = () => {
    setTouched({ email: true });
    setValidationErrors({ email: validateEmail(email) });
  };

  const handleChange = (e) => {
    setEmail(e.target.value);
    if (touched.email) {
      setValidationErrors({ email: validateEmail(e.target.value) });
    }
  };
        ...
        <div className="w-full h-full flex flex-col justify-center p-10 md:p-16 bg-white dark:bg-[#1a1a1a] relative border-l border-gray-100 dark:border-[#333] transition-colors">
          <div className="max-w-md mx-auto w-full">
            <div className="mb-10 text-center md:text-left">
              <h2 className="text-4xl font-bold text-[#D4AF37] mb-2 font-serif">Forgot Password</h2>
              <p className="text-gray-500 dark:text-gray-400 font-sans transition-colors">Enter your email to receive an OTP.</p>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['CITIZEN', 'LAWYER', 'NGO', 'ADMIN'].map((r) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setRole(r)}
                      className={`py-2.5 px-4 text-xs font-bold uppercase tracking-widest transition-all border ${role === r
              ...
              <div className="group">
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <FiMail className="h-5 w-5 text-gray-400 dark:text-gray-500 group-focus-within:text-[#D4AF37] transition-colors" />
                  </div>
                  <input
                    type="email"
                    value={email}
                    ...
                    onBlur={handleBlur}
                    className={`block w-full pl-12 pr-4 py-4 bg-gray-50 dark:bg-[#252525] border ${touched.email && validationErrors.email ? 'border-red-500 bg-red-50 dark:border-red-900 dark:bg-red-900/10' : 'border-gray-200 dark:border-[#333]'} text-gray-900 dark:text-white placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none focus:border-[#D4AF37] dark:focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37] transition-all font-sans text-sm`}
                    placeholder="Enter your registered email"
                  />
                </div>
                {touched.email && validationErrors.email && (
                  <p className="text-red-500 text-xs mt-1 font-sans">{validationErrors.email}</p>
                )}
              </div>
