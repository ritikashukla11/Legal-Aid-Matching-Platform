...
import { FiUser, FiCamera, FiEdit3, FiSave, FiCheck, FiMail, FiPhone, FiMapPin, FiCalendar, FiShield, FiX } from "react-icons/fi";
import {
...
export default function AdminProfile({
  ...
  const handleUpdateProfile = async () => {
    ...
    if (!profile.aadhaar || !/^\d{12}$/.test(profile.aadhaar)) {
      validationErrors.aadhaar = "Valid Aadhaar (12 digits) is required";
    }
    if (!profile.mobile || !/^\d{10}$/.test(profile.mobile)) {
      validationErrors.mobile = "Valid mobile (10 digits) is required";
    }
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || err.response?.data || err.message || "Failed to update profile. Please try again.";
  ...
  return (
    <div className="w-full bg-white dark:bg-[#0a0a0a] min-h-screen font-sans transition-colors duration-300 p-4 sm:p-10">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12">
        <div>
          <span className="text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.3em] mb-3 block">Administrator Personnel File</span>
          <h1 className="text-3xl sm:text-5xl font-bold text-gray-900 dark:text-white font-serif tracking-tight transition-colors">System Identity</h1>
          <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-lg mt-2 font-medium tracking-wide transition-colors">Manage high-level administrative credentials and regional sovereignty data.</p>
        </div>
        ...
        <div className="flex gap-4">
          {!isEditingProfile ? (
            <button
              ...
              className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333] text-[#D4AF37] px-8 py-3.5 rounded-xl font-black uppercase text-[10px] tracking-[0.2em] hover:bg-[#D4AF37] hover:text-black transition-all flex items-center gap-3 shadow-xl"
            >
              <FiEdit3 className="w-4 h-4" /> Edit Authority
            </button>
          ) : (
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333] rounded-2xl p-8 relative overflow-hidden group transition-colors">
            <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
              <FiShield size={100} className="text-[#D4AF37]" />
            </div>
            ...
            <div className="flex flex-col items-center">
              <div className="relative mb-6">
                ...
                {isEditingProfile && (
                  <label htmlFor="admin-photo-upload" className="absolute -bottom-2 -right-2 w-10 h-10 bg-[#D4AF37] text-black rounded-lg flex items-center justify-center cursor-pointer hover:scale-110 transition-transform shadow-xl">
                    <FiCamera size={18} />
                    <input id="admin-photo-upload" type="file" className="hidden" accept="image/*" onChange={(e) => {
              ...
              <span className="text-[10px] font-black text-[#D4AF37] uppercase tracking-[0.3em] bg-[#D4AF37]/10 px-3 py-1 rounded-full mb-6">Root Administrator</span>
              ...
              <div className="w-full space-y-3 pt-6 border-t border-gray-100 dark:border-[#333] transition-colors">
                <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-gray-500">
                  ...
                  <span className="text-gray-700 dark:text-gray-300 transition-colors">#AD-{(profile.aadhaar || '0000').slice(-4)}</span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-gray-500">
                  <span>Privilege Level</span>
                  <span className="text-gray-700 dark:text-gray-300 transition-colors">Level 4 (Full Audit)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333] rounded-2xl p-8 sm:p-12 shadow-2xl space-y-12 transition-colors">
            ...
            <section className="space-y-8">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-xl font-bold text-gray-900 dark:text-white font-serif tracking-tight transition-colors">Identification Data</h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                />
                <Input
                  label="Administrative Email (Read Only)"
                  value={profile.email}
                  disabled={true}
                  icon={<FiMail />}
                  hint="Encrypted Primary Root Identifier"
                />
              </div>
            </section>
            ...
            <section className="space-y-8 pt-8 border-t border-gray-100 dark:border-[#222] transition-colors">
              <div className="flex items-center gap-4">
                <div className="w-1.5 h-6 bg-[#D4AF37] rounded-full"></div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white font-serif tracking-tight transition-colors">Regional Sovereignty & Hub</h2>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <Input
                  label="Emergency Contact Threshold"
                ...
                <div className="md:col-span-2">
                  <Textarea
                    label="Physical Command Coordinates (Address)"
